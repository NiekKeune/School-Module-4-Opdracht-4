...
namespace GradesPrototype.Data
{
    ...
    public class Student: IComparable<Student>
    {
        ...
        {
            if (grade.StudentID == 0)       //Checks if the Grade already has a student by checking its STudentID, if it's zero then it means it has no student and it'll add it to the current student.
            {
                grade.StudentID = StudentID;
            }
            else
            {
                throw new ArgumentException("Grade", "The grade belongs to a different student.");
            }
        }
    }
    ...
    public class Teacher
    {
        ...
        {
            if (student.TeacherID == 0)             //If the student has no teacher (As in the Teacher ID is 0) the current teacher will add this student.
            {
                student.TeacherID = TeacherID;
            }
            else
            {
                throw new ArgumentException("Student", "This student is already assigned to a class.");
            }
        }

        public void RemoveFromClass(Student student)
        {
            if (student.TeacherID == TeacherID)     //The opposite of the EnrollInClass method, checks if the student's teacher is the same as the one trying to remove him, and if yes sets the TeacherID for the student to 0 meaning he will have no teacher and effictively be removed from the class.
            {
                student.TeacherID = 0;
            }
            else
            {
                throw new ArgumentException("Student", "This student is not assigned to this class");
            }
        }
    }
}
